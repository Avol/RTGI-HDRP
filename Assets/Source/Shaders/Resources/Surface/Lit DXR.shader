    ...
    SubShader
    {
		...
		Pass
		{
			...
			void ClosestHit(inout RayPayload rayPayload : SV_RayPayload, AttributeData attributeData : SV_IntersectionAttributes)
			{
				...
				//posInput.positionSS = payload.pixelCoord; // TODO this

				RayCone cone;
				cone.width			= 1.0f;
				cone.spreadAngle	= 1.0f;

				// Build the surfacedata and builtindata
				SurfaceData surfaceData;
				BuiltinData builtinData;
				bool isVisible;
				GetSurfaceAndBuiltinData(fragInput, viewWS, posInput, surfaceData, builtinData, currentVertex, cone, isVisible);

				// bsdf
				BSDFData bsdfData = ConvertSurfaceDataToBSDFData(posInput.positionSS, surfaceData);

				rayPayload.color		= bsdfData.diffuseColor;
				rayPayload.emission		= builtinData.emissiveColor;
